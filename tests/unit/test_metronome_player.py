...
def test_get_orders_returns_joined_orders(
    ...
    offline_metronome_player: MetronomePlayer,
):
    ...
    with patch.object(DoubleBattleOrder, "join_orders") as mock:
        ...
        result = offline_metronome_player._get_orders(default_double_battle)
...
def test_get_orders_returns_all_orders_when_cannot_tera(
    ...
    with patch.object(DoubleBattleOrder, "join_orders") as join_orders_mock:
        with patch.object(offline_metronome_player._tera_strategy, "filter_orders"):
            join_orders_mock.return_value = joined_orders

            result = offline_metronome_player._get_orders(battle)
...
def test_get_orders_returns_tera_orders_when_available(
    ...
    with patch.object(DoubleBattleOrder, "join_orders") as join_orders_mock:
        with patch.object(
            offline_metronome_player._tera_strategy,
            "filter_orders",
...
async def test_choose_move_raises_error_for_non_double_battle(
    ...
    with pytest.raises(TypeError):
        await offline_metronome_player.choose_move(battle)
...
async def test_choose_move_returns_random_order(
    ...
    with patch.object(offline_metronome_player, "_get_orders") as player_mock:
...
async def test_choose_move_fallback_when_no_orders(
    ...
    with patch.object(offline_metronome_player, "_get_orders") as mock:
